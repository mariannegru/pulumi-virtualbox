PROJECT_NAME := Virtualbox provider
include build/common.mk

PACK             := virtualbox
PACKDIR          := sdk
PROJECT          := github.com/mariannegru/pulumi-virtualbox
NODE_MODULE_NAME := @pulumi/virtualbox
...
build::
	go install -ldflags "-X github.com/mariannegru/pulumi-virtualbox/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${TFGEN}
	go install -ldflags "-X github.com/mariannegru/pulumi-virtualbox/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}
...
install::
	GOBIN=$(PULUMI_BIN) go install -ldflags "-X github.com/mariannegru/pulumi-virtualbox/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}
